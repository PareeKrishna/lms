    ...
    try {
      ...
      const tempProgressArray = await Promise.all(
        enrolledCourses.map(async (course) => {
          ...
          // Prefer totalLectures from API (derived from latest course data);
          // fall back to client-side calculation if not provided.
          let totalLectures = typeof data.totalLectures === 'number'
            ? data.totalLectures
              ...
              <tr key={index} className="border-b border-gray-500/20">
                <td className="md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <Line strokeWidth={1.5} percent={progressArray[index] ? (progressArray[index].lectureCompleted / progressArray[index].totalLectures * 100) : 0} className='bg-gray-300 rounded-full' />
                  </div>
                </td>
                ...
                <td className="px-4 py-3 max-sm:text-right">
                  <button className="px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white rounded cursor-pointer" onClick={() => navigate('/player/' + course._id)}>
